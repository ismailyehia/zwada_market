...
class AuthProvider with ChangeNotifier {
  ...
  Future<ResponseModel> login(String? phone, String? password) async {
    ...
    _loginErrorMessage = '';
    notifyListeners();
    print('Attempting login with phone: $phone, password: $password');
    ApiResponse apiResponse = await authRepo!.login(phone: phone, password: password);
    ...
    if (apiResponse.response != null &&
      ...
      if (token != null) {
        ...
        print('Fetched User Info: ${profileProvider.userInfoModel}');
      ...
      if (userInfoModel != null && userInfoModel.id != null) {
        await saveUserIdToSharedPreferences(userInfoModel.id.toString());
        print('Saved User ID to Shared Preferences: ${userInfoModel.id}');
      } else {
        print('User ID is null in UserInfoModel');
      }

      // Test test = Test();
      //   List<dynamic> data = await test.getData();
      ...
      responseModel = ResponseModel(token != null, 'verification', role: role);
    } else {
...
//////////zeyada ana 3amelha
  ...
  Future<void> addGuest(BuildContext context) async {
    ...
    ApiResponse apiResponse = await authRepo!.addGuest(fcmToken, guestId);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200 &&
        apiResponse.response?.data['guest']['id'] != null) {
      String guestId = '${apiResponse.response?.data['guest']['id']}';
      print(guestId);
      authRepo?.saveGuestId(guestId);
      // // Save the guestId to a local variable for later use
      // _guestId = guestId;
    }
  }
