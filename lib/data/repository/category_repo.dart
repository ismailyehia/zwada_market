...
class CategoryRepo {
  ...
  Future<ApiResponse> getCategoryProductList(String? categoryID, String type, String? name) async {

    try {
      final response = await dioClient!.get('${AppConstants.categoryProductUri}$categoryID?${name != null ? '&search=$name' : ''}');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
