...
class DateConverter {
  ...
  //   return DateFormat('HH:mm').format(isoStringToLocalTimee(dateTime));
  // }


static String isoStringToLocalTimeOnly(String dateTime) {
  // Parse the ISO string to DateTime object
  DateTime parsedDateTime = DateFormat('yyyy-MM-ddTHH:mm:ss.SSS').parse(dateTime, true);
  
  // Add an hour to the parsed DateTime object
  DateTime adjustedDateTime = parsedDateTime.add(const Duration(hours: 1));
  
  // Format the adjusted DateTime object to local time
  return DateFormat('HH:mm').format(adjustedDateTime.toLocal());
}



static DateTime isoStringToLocalTimee(String dateTime) {
  // Parse the ISO string to DateTime object
  DateTime parsedDateTime = DateFormat('yyyy-MM-ddTHH:mm:ss.SSS').parse(dateTime, true).toLocal();
  
  // Create a new DateTime object with only hours and minutes, setting other components to zero
  DateTime timeOnly = DateTime(parsedDateTime.year, parsedDateTime.month, parsedDateTime.day, parsedDateTime.hour, parsedDateTime.minute);
